import React, { useState } from "react";
import { useNavigate } from "react-router-dom";
import Navbar from "../components/Layout/Navbar";
import Footer from "../components/Layout/Footer";
import projectsCatalog from "../data/projects.json";
import InternshipDetailModal from "../components/UI/InternshipDetailModal";

const statusStyles = {
  Active: "bg-indigo-100 text-indigo-700 border border-indigo-200",
  "In-progress": "bg-amber-100 text-amber-700 border border-amber-200",
  Completed: "bg-emerald-100 text-emerald-700 border border-emerald-200",
};

const getInternshipKey = (project) =>
  [
    project?.position_overview?.title,
    project?.contact_email,
    project?.duration?.start_date,
  ]
    .filter(Boolean)
    .join("|");

const InternshipCatalogCard = ({
  project,
  onViewDetails,
  onApply,
  isWaiting = false,
  isLoading = false,
}) => {
  const status = project?.status || "Active";
  const badgeClasses =
    statusStyles[status] || "bg-slate-100 text-slate-700 border border-slate-200";

  return (
    <div className="relative overflow-hidden rounded-3xl bg-white shadow-lg shadow-primary-100/40 ring-1 ring-slate-100 transition hover:-translate-y-1 hover:shadow-2xl">
      <div className="absolute inset-0 bg-gradient-to-br from-primary-100/50 via-transparent to-sky-100/60" />
      <div className="relative space-y-5 p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${badgeClasses}`}>
              {status}
            </div>
            <h3 className="mt-4 text-2xl font-semibold text-slate-900">
              {project?.position_overview?.title}
            </h3>
            <p className="mt-3 text-sm text-slate-600 leading-relaxed max-w-3xl">
              {project?.position_overview?.summary}
            </p>
          </div>
          <div className="flex flex-col items-end text-sm text-slate-600">
            <span className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1 font-medium text-slate-700 shadow-sm">
              🗓 {project?.duration?.start_date} – {project?.duration?.end_date}
            </span>
            <span className="mt-2 inline-flex items-center gap-2 rounded-full bg-white px-3 py-1 font-medium text-slate-700 shadow-sm">
              ⏱ {project?.time_commitment}
            </span>
            <span className="mt-2 inline-flex items-center gap-2 rounded-full bg-white px-3 py-1 font-medium text-slate-700 shadow-sm">
              📍 {project?.location}
            </span>
          </div>
        </div>

        <div className="flex flex-wrap gap-3 pt-2">
          <button
            onClick={() => onViewDetails?.(project)}
            className="rounded-xl border border-primary-200 px-4 py-2 text-sm font-semibold text-primary-700 transition hover:border-primary-300 hover:bg-primary-50"
          >
            View details
          </button>
          {typeof onApply === "function" && status === "Active" && (
            <button
              onClick={() => onApply(project)}
              disabled={isWaiting || isLoading}
              className={`rounded-xl px-4 py-2 text-sm font-semibold text-white shadow ${
                isWaiting || isLoading
                  ? "bg-primary-300 cursor-not-allowed"
                  : "bg-primary-600 hover:bg-primary-700"
              }`}
            >
              {isLoading ? "Sending..." : isWaiting ? "Waiting for response" : "Apply"}
            </button>
          )}
        </div>
      </div>
    </div>
  );
};

export default function CandidateInternships() {
  const navigate = useNavigate();
  const storedCandidate = localStorage.getItem("candidate");
  const [selectedProject, setSelectedProject] = useState(null);
  const [appliedProjects, setAppliedProjects] = useState({});

  const currentUser = storedCandidate ? JSON.parse(storedCandidate) : null;

  if (!storedCandidate) {
    navigate("/");
    return null;
  }

  const candidateEmail = currentUser?.email || null;
  const candidateName = currentUser?.name || null;

  const handleViewDetails = (project) => {
    setSelectedProject(project);
  };

  const API_BASE_URL = import.meta.env.VITE_API_URL || "http://localhost:5000";

  const handleApply = async (project) => {
    const projectKey = getInternshipKey(project);
    if (!projectKey || appliedProjects[projectKey] === "waiting") {
      return;
    }

    const senderEmail = candidateEmail;
    if (!senderEmail) {
      alert("Please log in as a candidate to apply for internships.");
      return;
    }

    setAppliedProjects((prev) => ({
      ...prev,
      [projectKey]: "loading",
    }));

    try {
      const response = await fetch(`${API_BASE_URL}/api/send-application-email`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          senderEmail,
          candidateName,
          internshipTitle: project?.position_overview?.title,
        }),
      });

      if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        throw new Error(data?.detail || "Failed to send email");
      }

      alert("Mail sent to Scale Up team. They will get back to you shortly.");
      setAppliedProjects((prev) => {
        return { ...prev, [projectKey]: "waiting" };
      });
    } catch (error) {
      console.error(error);
      alert("Failed to send email. Please try again later.");
      setAppliedProjects((prev) => {
        const next = { ...prev };
        delete next[projectKey];
        return next;
      });
    }
  };

  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <Navbar />

      <div className="flex-1 bg-gradient-to-br from-primary-50/70 via-white to-sky-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="mb-10 flex flex-col gap-4 rounded-3xl bg-white/90 px-8 py-10 shadow-xl ring-1 ring-primary-100/60">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="text-sm uppercase font-semibold tracking-[0.3em] text-primary-500">
                  Opportunity explorer
                </p>
                <h1 className="mt-3 text-3xl font-bold text-slate-900">
                  Discover internships tailored for Scale Up talent
                </h1>
                <p className="mt-3 max-w-2xl text-sm text-slate-600">
                  Browse open positions curated from partner companies. Use your profile to apply and
                  gain hands-on experience.
                </p>
              </div>
              <button
                onClick={() => navigate("/candidate/dashboard")}
                className="inline-flex items-center justify-center rounded-2xl bg-primary-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-primary-600/30 transition hover:-translate-y-0.5 hover:bg-primary-700"
              >
                ← Back to dashboard
              </button>
            </div>
          </div>

          <div className="space-y-8">
            {(projectsCatalog?.internships || []).map((project, idx) => (
              <InternshipCatalogCard
                key={`${project?.position_overview?.title}-${idx}`}
                project={project}
                onViewDetails={handleViewDetails}
                onApply={handleApply}
                isWaiting={appliedProjects[getInternshipKey(project)] === "waiting"}
                isLoading={appliedProjects[getInternshipKey(project)] === "loading"}
              />
            ))}
          </div>
        </div>
      </div>

      <Footer />

      {selectedProject && (
        <InternshipDetailModal
          internship={selectedProject}
          onClose={() => setSelectedProject(null)}
          onApply={handleApply}
          isWaiting={appliedProjects[getInternshipKey(selectedProject)] === "waiting"}
          isLoading={appliedProjects[getInternshipKey(selectedProject)] === "loading"}
        />
      )}
    </div>
  );
}


